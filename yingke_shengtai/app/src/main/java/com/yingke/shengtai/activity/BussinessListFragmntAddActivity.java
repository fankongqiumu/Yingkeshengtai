 ...
 */
public class BussinessListFragmntAddActivity extends BaseActivity implements View.OnClickListener{
    ...
    private void sendData(String name, String phone, String contractid,
        ...
        if (TextUtils.equals("update", flag)){
            ...
            JosnUtil.gson.toJson(map);
            getData(IApi.NETWORK_METHOD_POST, TAG_UPDATE_BUSSINESS_LIST_FRAGMENT, IApi.URL_SALE_BUSSINESS_LIST_LIST + "/" + currentData.getDetail().getBid(), map);
        } else {
            map.put("action","insert");
            JosnUtil.gson.toJson(map);
            getData(IApi.NETWORK_METHOD_POST, TAG_ADD_BUSSINESS_LIST_FRAGMENT, IApi.URL_SALE_BUSSINESS_LIST_LIST /**+ "/" + currentData.getDetail().getBid()**/, map);
        }
