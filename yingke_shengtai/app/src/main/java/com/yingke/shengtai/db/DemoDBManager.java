...
import com.yingke.shengtai.db.InviteMessage.InviteMesageStatus;
...
public class DemoDBManager {
     ...
     * @param
     */
    synchronized public void saveContactData(User user) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
//            db.delete(UserDao.TABLE_NAME_INFOR, null, null);
                ContentValues values = new ContentValues();
                values.put(UserDao.COLUMN_NAME_ID, user.getUsername());
                if(user.getNick() != null)
                    values.put(UserDao.COLUMN_NAME_NICK, user.getNick());
     ...
     * @return
     */
    synchronized public Map<String, User> getContactMap() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Map<String, User> users = new HashMap<String, User>();
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TABLE_NAME_INFOR /* + " desc" */, null);
